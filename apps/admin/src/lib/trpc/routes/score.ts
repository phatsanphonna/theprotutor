...
export const scoreRoutes = t.router({
  createScoreboard: teacherProcedure.input(z.object({
    name: z.string(),
    total: z.number(),
  ...
  editScoreboard: teacherProcedure.input(z.object({
    id: z.number(),
    name: z.string(),
    total: z.number(),
  })).mutation(async ({ ctx, input }) => {
    ...
    const mean = scoreboard.scores.length === 0 ? 0 : scoreboard.scores.reduce((acc, curr) => acc + curr.score, 0) / scoreboard.scores.length;
    const min = scoreboard.scores.length === 0 ? 0 : Math.min(...scoreboard.scores.map((scores) => scores.score));
    const max = scoreboard.scores.length === 0 ? 0 : Math.max(...scoreboard.scores.map((scores) => scores.score));
    const sd = scoreboard.scores.length === 0 ? 0 : Math.sqrt(scoreboard.scores.reduce((acc, curr) => acc + Math.pow(curr.score - mean, 2), 0) / scoreboard.scores.length);
  ...
  updateStudentScore: teacherProcedure.input(z.object({
    id: z.number(),
    students: z.array(
      z.object({
        score: z.number(),
    ...
    const mean = students.length === 0 ? 0 : students.reduce((acc, curr) => acc + curr.score, 0) / students.length;
    ...
    const sd =  students.length === 0 ? 0 :Math.sqrt(students.reduce((acc, curr) => acc + Math.pow(curr.score - mean, 2), 0) / students.length);
  ...
  deleteStudentScore: teacherProcedure.input(z.object({
    id: z.number(),
    studentId: z.string(),
  })).mutation(async ({ ctx, input }) => {
    const { db } = ctx;
    ...
    const mean = scores.length === 0 ? 0 : scores.reduce((acc, curr) => acc + curr.score, 0) / scores.length;
    ...
    const sd = scores.length === 0 ? 0 : Math.sqrt(scores.reduce((acc, curr) => acc + Math.pow(curr.score - mean, 2), 0) / scores.length);
