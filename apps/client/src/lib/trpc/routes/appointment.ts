...
export const appointmentRoutes = t.router({
  ...
  getAppointmentById: authProcedure
    .input(z.number())
  ...
  checkValidAppointment: authProcedure
    .input(
      z.object({
    ...
    .query(async ({ ctx, input }) => {
      ...
      const parsedEndTime = new Date(
        parsedAppointmentTime.getTime() + endTime * 60 * 60 * 1000,
      );
  ...
  deleteAppointmentById: authProcedure
    .input(z.number())
  ...
  createAppointment: authProcedure
    .input(
      z.object({
        appointmentTime: z.string(),
        endTime: z.number(),
